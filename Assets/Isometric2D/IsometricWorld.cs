...
public class IsometricWorld : MonoBehaviour
{
    ...
    public Vector3 DirectionToRightTop
    {
        get
        {
            if (_cachedDirectionToRightTop == default)
                _cachedDirectionToRightTop = (IsometricIdentityCorners[0] - IsometricIdentityCorners[3]).normalized;
            return _cachedDirectionToRightTop;
        }
    }

    public Vector3 DirectionToLeftTop
    {
        get
        {
            if (_cachedDirectionToLeftTop == default)
                _cachedDirectionToLeftTop = (IsometricIdentityCorners[0] - IsometricIdentityCorners[1]).normalized;
            return _cachedDirectionToLeftTop;
        }
    }

    public Vector3 DirectionToLeftBottom
    {
        get
        {
            if (_cachedDirectionToLeftBottom == default)
                _cachedDirectionToLeftBottom = (IsometricIdentityCorners[2] - IsometricIdentityCorners[1]).normalized;
            return _cachedDirectionToLeftBottom;
        }
    }

    public Vector3 DirectionToRightBottom
    {
        get
        {
            if (_cachedDirectionToRightBottom == default)
                _cachedDirectionToRightBottom = (IsometricIdentityCorners[1] - IsometricIdentityCorners[0]).normalized;
    ...
    {
        var extendToRightTop = (IsometricIdentityCorners[0] - IsometricIdentityCorners[3]) * (extends[0] - 1.0f);
        var extendToLeftTop = (IsometricIdentityCorners[0] - IsometricIdentityCorners[1]) * (extends[1] - 1.0f);
